...
function projectPatch(project: Project, input: ProjectInput): Partial<ProjectInput> {
  const patch: Partial<ProjectInput> = {};

  if (input.number !== project.number) patch.number = input.number;
  if (input.slug !== project.slug) patch.slug = input.slug;
  if (input.title !== project.title) patch.title = input.title;
  if (input.label !== project.label) patch.label = input.label;
  if (input.description !== project.description) patch.description = input.description;
  if (input.github !== project.github) patch.github = input.github;
  if (input.visualKey !== project.visualKey) patch.visualKey = input.visualKey;
  if (input.iconKey !== project.iconKey) patch.iconKey = input.iconKey;
  if (input.isFeatured !== project.isFeatured) patch.isFeatured = input.isFeatured;
  if (input.displayOrder !== project.displayOrder) patch.displayOrder = input.displayOrder;
  if (!sameLines(input.highlights, project.highlights)) patch.highlights = input.highlights;
  if (!sameLines(input.tech, project.tech)) patch.tech = input.tech;
